...
namespace testfrom
{
    class socket_server
    {
        ...
        protected delegate void UpdateShowItem(string diIP, string vaIP);
        public ListBox lbshow;
        public TextBox tbshow;
        private BindingList<ClientInfo> lblist = new BindingList<ClientInfo>();
        ...
        protected void updateControlItem(string diIP,string vaIP)
        {
            ClientInfo clientip = new ClientInfo();
            clientip.DisIp = diIP;
            clientip.ValueIp = vaIP;
            lblist.Add(clientip);
            this.lbshow.DataSource =lblist;
            this.lbshow.ValueMember = "ValueIp";
            this.lbshow.DisplayMember = "DisIp";
            return;
        }
        /// <summary>
        /// 启动服务
        ...
        public bool OpenServer(int port)
        {
            try
            {
                ...
                try
                {
                    ...
                    tbshow.Text += String.Format("开始监听：{0}:{1}\r\n", IPAddress.Any.ToString(),port.ToString());
        ...
        public void CloseServer()
        {
            ...
            if (ServerSocket != null)
                ServerSocket.Close();
            lblist.Clear();
            tbshow.Text += String.Format("服务已停止\n");
        }
        /// <summary>
        /// 监听客户端请求的方法；
        /// </summary>
        private void ListenConnecting()
        {
            while (Flag_Listen)  // 持续不断的监听客户端的连接请求；
            {
                try
                {
                    ...
                    UpdateShowItem upItem = new UpdateShowItem(updateControlItem);
                    
                    string str_EndPoint = sokConnection.RemoteEndPoint.ToString();
                    if (dic_ClientSocket.Keys.Count<string>(p=>p.Split(':')[0]==str_EndPoint.Split(':')[0])>0)
                    {
                        sokConnection.Shutdown(SocketShutdown.Both);
                        this.tbshow.Invoke(up, String.Format("客户端：{0} 重复连接\r\n", str_EndPoint));
                        continue;
                    }
                    ...
                    this.tbshow.Invoke(up, String.Format("客户端：{0} 已连接\r\n", myTcpClient.GetIp()));
                    this.lbshow.Invoke(upItem,myTcpClient.GetIp(),str_EndPoint);
                    SendData(str_EndPoint, Encoding.Default.GetBytes("连接成功\n"));
                }
                catch
                {
